...
M.notify = function(msg, level, opts)
  ...
  local nopts = { title = "Oh My Monokai" }
  if opts.once then
...
M.load = function(hl_groups)
  ...
  vim.g.colors_name = "oh-my-monokai"

  draw(hl_groups)

  local bufferline_icon_group = require("oh-my-monokai.theme.plugins.bufferline").setup_bufferline_icon()
  draw(bufferline_icon_group)
  -- draw bufferline icons
  vim.api.nvim_create_autocmd({ "BufEnter", "BufReadPost", "BufWinEnter", "BufRead" }, {
    pattern = "*",
    callback = function()
      if vim.g.colors_name ~= "oh-my-monokai" then
        return true
      end
      bufferline_icon_group = require("oh-my-monokai.theme.plugins.bufferline").setup_bufferline_icon()
      draw(bufferline_icon_group)
    end,
  })
end
